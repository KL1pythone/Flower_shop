...
API_TOKEN = '7380866413:AAFTn1siHjo0lCOJxXPuL_EoadnUVKg2QAo'
YOOKASSA_SHOP_ID = '460338'
YOOKASSA_SECRET_KEY = 'live_be5ZOcf-IzLArC0VzyxkYVPijzVN8a-ViHeY_qJXpeQ'
...
def get_flowers_catalog():
    ...
    response = requests.get(url="https://store.tildaapi.com/api/getproductslist/?storepartuid=811512390002&recid=596032466&c=1726771242496&getparts=true&getoptions=true&slice=1&filters%5Bquantity%5D=y&sort%5Bcreated%5D=desc&size=100", headers=headers)
...
def init_excel():
    try:
        load_workbook('orders_2.xlsx')
    except (FileNotFoundError, InvalidFileException):
        wb = Workbook()
        ws = wb.active
        ws.title = "Orders"
        ws.append(['ID', 'Flower Number', 'Phone Number', 'Name', 'Date Time', 'Payment Confirmed'])
        wb.save('orders_2.xlsx')

def save_crm(amount, flower_number, name, phone_number):
...
def save_order(flower_number, phone_number, name, payment_confirmed):
    wb = openpyxl.load_workbook('orders_2.xlsx')
    ws = wb.active
...
async def process_flower_number(message: types.Message, state: FSMContext):
    ...
    if flower_number in flowers:
        ...
        await message.answer(
            'К сожалению, этот букет уже приобрели или вы указали неверный номер, выберите пожалуйста другой номер или посмотрите те, что доступны к выбору на <a href="https://Izbata.cvetoot.ru/">сайте</a>',
